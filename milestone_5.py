...
class Hangman():
    ...
    def _play_game(self, list_of_fruits):
        '''
    ...
    def _ask_for_valid_user_input(self):  #this is already been accessed in the play_game method so you don't need to call it again ask_for_valid_user_input()
                                          # as this will make it a recursive function
        '''
        ...
        while True:  # let the code run continuosly 
            ...
            if self.validate_guess(guess) and not self.already_guessed(guess):  #calling a method of an object where `self` is the object
...
num_lives = 5
game = Hangman(randomly_generated_fruit, [], len(set(randomly_generated_fruit)), list_of_fruits, [], num_lives=5)
print(game._play_game(list_of_fruits))
print(game._initialise_word_guessed())
